        ...
        for t in 0..threads {
            ...
            handles.push(std::thread::spawn(move || {
                ...
                for trial_num in (t..trials).step_by(threads) {
                    let (optimized_time, result) =
                        run_trial(trial_num, rounds, start_seed, &mut optimizer, t);
...
fn select_optimizer(trial_name: &str, ballot: &ballot::Ballot) -> Box<dyn Optimizer> {
    match trial_name {
        ...
        "minimax-friends" => Box::new(optimizers::mcmc::minimax_friends::MinimaxFriends::new(
            ballot,
        )),
        ...
        "network" => Box::new(optimizers::network::NetworkOptimizer::new(
            ballot, 10.0, 10.0,
        )),
...
fn run_trial(
    ...
    thread_num: usize,
) -> (Duration, Vec<Vec<Student>>) {
    crate::log_info!(
        format!("starting trial {} with {} rounds", trial, rounds),
        format!("optimizer-{}", thread_num)
    );
