...
impl MinimaxFriends {
    ...
    fn step(
        &self,
        mut schedule: Vec<Vec<Student>>,
        mut student_locations: HashMap<usize, usize>,
    ) -> (Vec<Vec<Student>>, HashMap<usize, usize>) {
...
impl MCMCOptimizer for MinimaxFriends {
    ...
    fn acceptance(&self, schedule: &Vec<Vec<Student>>, proposal: Proposal) -> f64 {
        ...
        if schedule[proposal.proposed_house].len()
            >= self.ballots.houses[proposal.proposed_house].capacity + extra_room_constant
        {
            return 0.0;
        }
