...
mod tests {
    ...
    fn test_swap_naive() {
        ...
        assert!(optimizers::validate_ballot(
            &input_ballot,
            swap_naive.optimize(1)
        ));
        assert!(optimizers::validate_ballot(
            &input_ballot,
            swap_naive.optimize(10)
        ));
        assert!(optimizers::validate_ballot(
            &input_ballot,
            swap_naive.optimize(100)
        ));
    }
    ...
    fn test_minimax_swap() {
        ...
        assert!(optimizers::validate_ballot(
            &input_ballot,
            minimax_swap.optimize(0)
        ));
        assert!(optimizers::validate_ballot(
            &input_ballot,
            minimax_swap.optimize(1)
        ));
        assert!(optimizers::validate_ballot(
            &input_ballot,
            minimax_swap.optimize(100)
        ));
    }
    ...
    fn test_minimax_friends() {
        ...
        let mut minimax_friends =
            optimizers::mcmc::minimax_friends::MinimaxFriends::new(&input_ballot);

        assert!(optimizers::validate_ballot(
            &input_ballot,
            minimax_friends.optimize(0)
        ));
        assert!(optimizers::validate_ballot(
            &input_ballot,
            minimax_friends.optimize(1)
        ));
        assert!(optimizers::validate_ballot(
            &input_ballot,
            minimax_friends.optimize(100)
        ));
    }
}
